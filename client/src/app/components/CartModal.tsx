      ...
      {cartModal && (
        <div className="w-[300px] h-[100vh] overflow-y-auto fixed right-0 bg-white border border-l-gray-700 pb-16">
          ...
          <div className="p-4">
            ...
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <div key={product.id} className="border-b py-2">
                  ...
                  <button className="mt-2 w-6 h-6 bg-red-950 hover:bg-red-600 p-2 rounded-lg cursor-pointer flex items-center justify-center"  onClick={() => removeFromCart(product.id)}>
                    <FaTrash />
                  </button>
                  </div>
            ...
            <button className="w-full grid mt-5 self-end place-content-center bg-orange-950 hover:bg-[#2b0c04] rounded-md text-white p-2" onClick={(e) => handleNewRequest(e)} >
              {!buttonLoading?'Prossiga para o pagamento' : <div className='w-[30px] h-[30px]'></div>}
            </button>
          </div>
        </div>
      )}
